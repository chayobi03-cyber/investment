...
async function insertRows(rows: Record<string, unknown>[]): Promise<void> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Supabase insert failure: ${response.status} ${text}`);
  }
}

Deno.serve(async (req: Request) => {
  try {
    const expectedSecret = Deno.env.get("MARKET_COLLECTOR_SECRET");
    if (!expectedSecret) throw new Error("Missing required secret: MARKET_COLLECTOR_SECRET");
    ...
    for (const symbol of symbols) {
      try {
        ...
        const observedAt = observationTimestamp(output, availableAt);
        rows.push({
          run_id: runId,
          ...
          symbol,
          market_session: String(output.new_mkop_cls_code ?? output.market_cls_code ?? "UNKNOWN"),
      ...
      // KIS documents 18 REST requests/sec for live accounts. Keep calls separated
      // to avoid bursty scheduling and to preserve an auditable acquisition rhythm.
    ...
    return new Response(JSON.stringify({
      ...
      status: Object.keys(failures).length === 0 ? 200 : 207,
      headers: { "content-type": "application/json" },
    });
  } catch (error) {
    return new Response(JSON.stringify({
