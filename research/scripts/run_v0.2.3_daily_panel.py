...
for name,start,end,anchor,kind in WINDOWS:
    for stage,(state_col,onset_col) in stages.items():
        onset=panel[onset_col]; trig=first_true(onset,start,end); pre=first_true(onset,start,end,anchor) if kind=='CRISIS' else None; coverage_ok=not (name.startswith('1994') and stage=='Crisis Confirmation')
...
for stage in stages:
    ...
    fp_hits=fp['hit'].fillna(False).astype(bool); crisis_hits=crisis['hit'].fillna(False).astype(bool); leadvals=crisis.loc[crisis_hits,'lead_days'].dropna(); timingvals=crisis['timing_vs_anchor_days'].dropna(); episodes=sum(int(panel[stages[stage][1]].loc[w[1]:w[2]].sum()) for w in WINDOWS)
