...
**중요 — 이 §4의 매수강도는 시장 전체(macro) 레짐 판단이며, 개별 종목 매수
판단에 그대로 대입하지 않는다.** 종목별 실제 매매 판단은
`docs/governance/PER_ASSET_BUY_INTENSITY_RISK_SCORE_FRAMEWORK_v0.1.md`의
종목별 매수강도/리스크 점수(두 개의 독립된 숫자)로 별도 산출한다.
