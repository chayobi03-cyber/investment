...
def load(name: str, path: Path):
    ...
    module = importlib.util.module_from_spec(name, spec)
    spec.loader.exec_module(module)
    return module


def test_event_edge_uses_net_dividend():
    mod = load("dividend_event", ROOT / "scripts/market_monitor/dividend_event_p0p6.py")
...
def test_event_window_excludes_ex_date_as_entry():
    mod = load("dividend_event2", ROOT / "scripts/market_monitor/dividend_event_p0p6.py")
    rows = [
...
def test_missing_dividend_fails_closed():
    mod = load("dividend_event3", ROOT / "scripts/market_monitor/dividend_event_p0p6.py")
    with tempfile.TemporaryDirectory() as td:
        p = Path(td) / "x.jsonl"
        p.write_text(json.dumps({"asset_id": "SAMSUNG_ELECTRONICS", "date": "2026-09-22"}) + "\n")
        result = mod.run(p, gross_dividend=None)
